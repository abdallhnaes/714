...
public class MainActivity extends Activity {
    ...
    private Team teamA = new Team("العمران",
    ...
    protected void onCreate(Bundle b) {
        super.onCreate(b);
        sp = getSharedPreferences("derby_pro_v4", MODE_PRIVATE);
        load();
        showHome();
    }

    private void load() {
        teamA.name = sp.getString("a_name", teamA.name);
        teamB.name = sp.getString("b_name", teamB.name);
        teamA.formation = sp.getString("a_form", teamA.formation);
        teamB.formation = sp.getString("b_form", teamB.formation);
        teamA.substitute = sp.getString("a_sub", teamA.substitute);
        teamB.substitute = sp.getString("b_sub", teamB.substitute);
        for (int i=0;i<8;i++) {
            teamA.players[i] = sp.getString("a_p"+i, teamA.players[i]);
            teamB.players[i] = sp.getString("b_p"+i, teamB.players[i]);
        }
    }

    private void save() {
        SharedPreferences.Editor e = sp.edit();
        e.putString("a_name", teamA.name);
        e.putString("b_name", teamB.name);
        e.putString("a_form", teamA.formation);
        e.putString("b_form", teamB.formation);
        e.putString("a_sub", teamA.substitute);
        e.putString("b_sub", teamB.substitute);
        for (int i=0;i<8;i++) {
            e.putString("a_p"+i, teamA.players[i]);
            e.putString("b_p"+i, teamB.players[i]);
        }
        e.apply();
    }
    ...
    private void header(String active) {
        ...
        TextView title = text("DERBY PRO", 26, Color.WHITE, Typeface.BOLD);
        TextView sub = text("إدارة خطة الفريقين ونتائج الديربي", 13, Color.LTGRAY, Typeface.BOLD);
        top.addView(title);
        top.addView(sub);
    ...
    private View scoreCard(String title, String num, String label) {
        ...
        TextView a = text(title, 13, Color.WHITE, Typeface.BOLD);
        TextView b = text(num, 24, GOLD, Typeface.BOLD);
        TextView c = text(label, 11, Color.LTGRAY, Typeface.BOLD);
        card.addView(a);
        card.addView(b);
        card.addView(c);
        LinearLayout.LayoutParams m = new LinearLayout.LayoutParams(-1, -1);
        m.setMargins(dp(3),0,dp(3),0);
        card.setLayoutParams(m);
        return card;
    }
    ...
    private void showHome() {
        ...
        TextView title = text("خطة الفريقين قبل المباراة", 20, Color.WHITE, Typeface.BOLD);
        box.addView(title);

        TacticsView viewA = new TacticsView(this);
        viewA.setTeam(teamA, false, true);
        ...
        viewB.setTeam(teamB, false, true);
    ...
    private void showTeamEditor(boolean first) {
        ...
        TextView labelName = text("اسم الفريق", 14, GOLD, Typeface.BOLD);
        labelName.setGravity(Gravity.RIGHT);
        box.addView(labelName);
        EditText teamName = input(team.name);
        box.addView(teamName, new LinearLayout.LayoutParams(-1, dp(52)));

        TextView labelForm = text("الخطة", 14, GOLD, Typeface.BOLD);
        labelForm.setGravity(Gravity.RIGHT);
        box.addView(labelForm);
        Spinner spinner = new Spinner(this);
        String[] forms = {"3-3-1", "3-2-2", "2-3-2", "1-3-3"};
        ...
        String[] labels = {"الحارس","مدافع يسار","مدافع وسط","مدافع يمين","وسط يسار","محور / أمام المدافعين","وسط يمين","رأس حربة"};
        EditText[] inputs = new EditText[8];
        for (int i=0;i<8;i++) {
            TextView l = text(labels[i], 14, GOLD, Typeface.BOLD);
        ...
        TextView lsub = text("البديل", 14, GOLD, Typeface.BOLD);
        lsub.setGravity(Gravity.RIGHT);
        box.addView(lsub);
        EditText sub = input(team.substitute);
        box.addView(sub, new LinearLayout.LayoutParams(-1, dp(52)));

        Button save = button("حفظ خطة الفريق");
        save.setOnClickListener(v -> {
            ...
            save();
            hideKeyboard();
            Toast.makeText(this, "تم حفظ خطة الفريق", Toast.LENGTH_SHORT).show();
            showHome();
        });
    ...
    private void showAddResult() {
        ...
        TextView teams = text(teamA.name + "  ×  " + teamB.name, 22, Color.WHITE, Typeface.BOLD);
        box.addView(teams);
        ...
        box.addView(sectionTitle("تشكيلة " + teamA.name));
        box.addView(v1, new LinearLayout.LayoutParams(-1, dp(600)));
        box.addView(sectionTitle("تشكيلة " + teamB.name));
        box.addView(v2, new LinearLayout.LayoutParams(-1, dp(600)));

        Button saveResult = button("حفظ النتيجة وتحديث عدد الانتصارات");
    ...
    public static class TacticsView extends View {
        ...
        private void drawPoster(Canvas c, int w, int h, Team team, boolean arrows, boolean footer) {
            int gold = Color.rgb(216,180,90);
            p.setStyle(Paint.Style.FILL);
            p.setColor(Color.rgb(6,16,12));
            c.drawRect(0,0,w,h,p);

            RectF pitch = new RectF(w*.07f, h*.06f, w*.93f, h*(footer?.82f:.96f));
            drawStadium(c, pitch);
            drawPitch(c, pitch);

            p.setTextAlign(Paint.Align.CENTER);
            p.setTypeface(Typeface.DEFAULT_BOLD);
            p.setTextSize(h*.038f);
            p.setColor(Color.WHITE);
            c.drawText(team.name + "  |  " + team.formation, w/2f, h*.045f, p);

            Pos[] ps = positions(pitch, team.formation);
            if (arrows) drawArrows(c, ps);

            int[] nums = {1,2,3,4,5,6,7,8};
            for (int i=0;i<8;i++) drawPlayer(c, ps[i].x, ps[i].y, nums[i], team.players[i]);
        ...
        private Pos[] positions(RectF r, String formation) {
            if ("3-2-2".equals(formation)) {
                return new Pos[]{P(r,.50f,.91f),P(r,.22f,.74f),P(r,.50f,.74f),P(r,.78f,.74f),P(r,.35f,.54f),P(r,.65f,.54f),P(r,.38f,.30f),P(r,.62f,.30f)};
            } else if ("2-3-2".equals(formation)) {
                return new Pos[]{P(r,.50f,.91f),P(r,.32f,.74f),P(r,.68f,.74f),P(r,.26f,.55f),P(r,.50f,.55f),P(r,.74f,.55f),P(r,.38f,.30f),P(r,.62f,.30f)};
            } else if ("1-3-3".equals(formation)) {
                return new Pos[]{P(r,.50f,.91f),P(r,.50f,.74f),P(r,.25f,.54f),P(r,.50f,.54f),P(r,.75f,.54f),P(r,.27f,.30f),P(r,.50f,.30f),P(r,.73f,.30f)};
            }
            return new Pos[]{P(r,.50f,.91f),P(r,.22f,.74f),P(r,.50f,.74f),P(r,.78f,.74f),P(r,.30f,.50f),P(r,.50f,.58f),P(r,.70f,.50f),P(r,.50f,.28f)};
        }

        private Pos P(RectF r, float x, float y) { return new Pos(r.left+r.width()*x, r.top+r.height()*y); }
        ...
        private void drawArrows(Canvas c, Pos[] ps) {
            int yellow=Color.rgb(255,215,32), red=Color.rgb(255,70,45);
            for(int i=1;i<ps.length;i++) arrow(c,ps[i].x,ps[i].y-36,ps[i].x,ps[i].y-115,yellow,false);
            arrow(c,ps[0].x,ps[0].y-20,ps[0].x-70,ps[0].y-90,yellow,false);
            arrow(c,ps[0].x,ps[0].y-20,ps[0].x+70,ps[0].y-90,yellow,false);
            for(int i=2;i<7;i++) arrow(c,ps[i].x,ps[i].y+30,ps[i].x,ps[i].y+95,red,true);
        }

        private void arrow(Canvas c,float x1,float y1,float x2,float y2,int color,boolean dashed){
            p.setStyle(Paint.Style.STROKE);
            p.setStrokeWidth(5);
            ...
            Path head=new Path();
            head.moveTo(x2,y2);
            head.lineTo((float)(x2-len*Math.cos(a-Math.PI/6)),(float)(y2-len*Math.sin(a-Math.PI/6)));
            head.lineTo((float)(x2-len*Math.cos(a+Math.PI/6)),(float)(y2-len*Math.sin(a+Math.PI/6)));
            head.close();
            p.setStyle(Paint.Style.FILL);
            p.setColor(color);
        ...
        static class Pos { float x,y; Pos(float a,float b){x=a;y=b;} }
    }
}
